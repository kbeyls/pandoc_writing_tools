...
local function translate_index_entry_to_latex(entry_text_parts)
  --logging.error('Translating index entry to latex: ', entry_text_parts);
  local latex_index_entry = '\\index{'
  for i, part in ipairs(entry_text_parts) do
    if i > 1 then
    ...
    latex_index_entry = latex_index_entry .. '}'
  --logging.error('Translated index entry to latex: ', latex_index_entry);
  return pandoc.RawInline('latex', latex_index_entry)
end
...
local function find_index_entries_in_text(span)
  ...
  -- First, split the text on semicolons, as these are used to indicate
  -- different index entries.
  local entries = split_string(text, ';')
  local latex_index_entries = {}
  for _, entry in ipairs(entries) do
    ...
    for i, part in ipairs(parts) do
      ...
      table.insert(latex_index_entries, translate_index_entry_to_latex(parts))
    end
  end
  if FORMAT:match 'latex' then
    table.insert(latex_index_entries, 1, span)
