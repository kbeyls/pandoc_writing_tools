...
function process_divs (div)
  ...
  if div.classes[1] == "definition" or div.classes[1] == "example" then
    ...
    if FORMAT:match 'latex' then
      ...
      --end
      if not div.attributes['caption'] then
        div.attributes['caption'] = label;
      else
        div.attributes['caption'] = label .. ": " .. div.attributes['caption'];
      end
      if FORMAT:match 'html' then
        table.insert(div.content, 1, pandoc.Span(div.attributes['caption'], {class = 'caption'}));
      end
      return div;
    end
  end
